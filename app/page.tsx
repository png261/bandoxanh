...
const FeatureCard: React.FC<FeatureCardProps> = ({ icon, title, description, page, navigateTo }) => (
...
interface NewsPreviewCardProps {
  article: NewsArticle;
  navigateTo: (page: string, options?: { articleId?: number }) => void;
}

const NewsPreviewCard: React.FC<NewsPreviewCardProps> = ({ article, navigateTo }) => (
  <div 
    onClick={() => {
...
const HomePageComponent: React.FC<HomePageProps> = ({ navigateTo }) => {
      ...
      <section>
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {latestNews.map(article => (
            <NewsPreviewCard key={article.id} article={article} navigateTo={navigateTo} />
          ))}
        </div>
    ...
    <div className="bg-brand-gray-light dark:bg-black min-h-screen font-sans text-brand-gray-dark dark:text-gray-200">
      ...
      <div className={`pt-20 md:pt-0 transition-all duration-300 ${isSidebarCollapsed ? 'md:pl-24' : 'md:pl-72'}`}> 
        <main className="container mx-auto px-4 sm:px-6 py-10">
          <HomePageComponent navigateTo={navigateTo} />
        </main>
        <Footer />
      </div>
    </div>
