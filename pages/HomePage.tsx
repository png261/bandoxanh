...
const FeatureCard: React.FC<{ icon: React.ReactNode; title: string; description: string; page: string; navigateTo: (page: string) => void; isAuthenticated: boolean; }> = ({ icon, title, description, page, navigateTo, isAuthenticated }) => (
  <div className="bg-card dark:bg-brand-gray-dark border border-gray-200 dark:border-gray-700 p-5 sm:p-6 md:p-8 rounded-xl hover:border-brand-green dark:hover:border-brand-green transition-all duration-300 flex flex-col">
    <div className="flex-shrink-0">{icon}</div>
    <div className="flex-grow mt-4 sm:mt-5">
      <h3 className="text-lg sm:text-xl font-semibold text-foreground dark:text-gray-100 break-words">{title}</h3>
      <p className="mt-2 text-sm sm:text-base text-brand-gray-DEFAULT dark:text-gray-400 break-words">{description}</p>
    </div>
    <div className="mt-6">
      {isAuthenticated ? (
        <button
...
const NewsPreviewCard: React.FC<{ article: NewsArticle; navigateTo: (page: string, options?: { articleId?: number }) => void; isAuthenticated: boolean; }> = ({ article, navigateTo, isAuthenticated }) => (
  ...
  >
    ...
    <div className="p-4 sm:p-5 flex flex-col flex-grow">
      ...
      <p className="text-xs sm:text-sm text-brand-gray-DEFAULT dark:text-gray-400 mt-2 sm:mt-3 line-clamp-2 break-words">{article.excerpt}</p>
    </div>
  </div>
);
...
const HomePage: React.FC<HomePageProps> = ({ navigateTo, isAuthenticated = false }) => {
  ...
  return (
    <div className="space-y-20 md:space-y-32">
      {/* Banner */}
      <section className="bg-brand-green-light dark:bg-brand-gray-dark text-center py-12 sm:py-16 md:py-20 px-4 sm:px-6 rounded-xl">
        <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold leading-tight text-brand-green-dark dark:text-white break-words">Chung Tay Vì Một Việt Nam Xanh</h2>
        <p className="mt-4 sm:mt-6 max-w-2xl mx-auto text-base sm:text-lg text-brand-gray-dark dark:text-gray-300 break-words px-2">
          {isAuthenticated 
      ...
      <section>
        <div className="text-center mb-8 sm:mb-12 px-4">
          <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-foreground dark:text-white">Tính năng nổi bật</h2>
          <p className="text-sm sm:text-base text-brand-gray-DEFAULT dark:text-gray-400 mt-2 sm:mt-3 max-w-xl mx-auto break-words">Các công cụ mạnh mẽ giúp bạn hành động vì môi trường, ngay trong tầm tay.</p>
        </div>
      ...
      <section>
        <div className="text-center mb-8 sm:mb-12 px-4">
          <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-foreground dark:text-white">Làm thế nào để bắt đầu?</h2>
          <p className="text-sm sm:text-base text-brand-gray-DEFAULT dark:text-gray-400 mt-2 sm:mt-3 break-words">Chỉ với 3 bước đơn giản để cùng BandoXanh hành động.</p>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 sm:gap-8 text-center">
          <div className="p-4 sm:p-6">
            <div className="flex items-center justify-center h-20 w-20 sm:h-24 sm:w-24 mx-auto bg-brand-green-light dark:bg-brand-gray-dark rounded-full">
              <MapPinIcon className="h-10 w-10 sm:h-12 sm:w-12 text-brand-green-dark dark:text-brand-green" />
            </div>
            <h3 className="mt-4 sm:mt-6 text-lg sm:text-xl font-semibold">1. Tìm kiếm</h3>
            <p className="mt-2 text-sm sm:text-base text-brand-gray-DEFAULT dark:text-gray-400 break-words">Sử dụng bản đồ tương tác để tìm các trạm thu gom rác tái chế gần bạn.</p>
          </div>
          <div className="p-4 sm:p-6">
            <div className="flex items-center justify-center h-20 w-20 sm:h-24 sm:w-24 mx-auto bg-brand-green-light dark:bg-brand-gray-dark rounded-full">
              <RecycleIcon className="h-10 w-10 sm:h-12 sm:w-12 text-brand-green-dark dark:text-brand-green" />
            </div>
            <h3 className="mt-4 sm:mt-6 text-lg sm:text-xl font-semibold">2. Phân loại</h3>
            <p className="mt-2 text-sm sm:text-base text-brand-gray-DEFAULT dark:text-gray-400 break-words">Chụp ảnh rác thải để AI của chúng tôi giúp bạn nhận diện và hướng dẫn xử lý.</p>
          </div>
          <div className="p-4 sm:p-6">
            <div className="flex items-center justify-center h-20 w-20 sm:h-24 sm:w-24 mx-auto bg-brand-green-light dark:bg-brand-gray-dark rounded-full">
              <CommunityIcon className="h-10 w-10 sm:h-12 sm:w-12 text-brand-green-dark dark:text-brand-green" />
            </div>
            <h3 className="mt-4 sm:mt-6 text-lg sm:text-xl font-semibold">3. Chia sẻ</h3>
            <p className="mt-2 text-sm sm:text-base text-brand-gray-DEFAULT dark:text-gray-400 break-words">Tham gia cộng đồng, chia sẻ hành động của bạn và lan tỏa cảm hứng sống xanh.</p>
          </div>
        </div>
      </section>
      ...
      <section className="bg-card dark:bg-brand-gray-dark border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden">
        <div className="grid grid-cols-1 md:grid-cols-2 items-center">
          <div className="p-6 sm:p-8 md:p-10 lg:p-12 order-2 md:order-1">
            <h2 className="text-2xl sm:text-3xl font-bold text-foreground dark:text-white break-words">Vì một tương lai xanh hơn</h2>
            <p className="mt-3 sm:mt-4 text-sm sm:text-base text-brand-gray-DEFAULT dark:text-gray-400 break-words">
            ...
            {isAuthenticated ? (
              <button 
                ...
                className="mt-6 sm:mt-8 w-full sm:w-auto bg-brand-green text-white font-semibold py-3 px-6 rounded-lg hover:bg-brand-green-dark transition-all transform hover:scale-105 flex items-center justify-center group text-sm sm:text-base"
              >
      ...
      <section>
        <div className="text-center mb-8 sm:mb-12 px-4">
          <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-foreground dark:text-white">Tin tức mới nhất</h2>
          <p className="text-sm sm:text-base text-brand-gray-DEFAULT dark:text-gray-400 mt-2 sm:mt-3 break-words">Cập nhật các thông tin và sự kiện môi trường nổi bật.</p>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6 md:gap-8">
